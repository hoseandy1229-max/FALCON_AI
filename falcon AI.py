...
# مدیریت امنیت بخش مدیریت
if "auth" not in st.session_state: st.session_state.auth = False

st.title("𝑭𝑨𝑳𝑪𝑶𝑵 𝑨𝑰")

# --- منوی مدیریت در سایدبار ---
with st.sidebar:
    ...
    if not st.session_state.auth:
        ...
        st.success("وارد شده‌اید")
        if st.button("خروج از مدیریت"):
            st.session_state.auth = False
            st.rerun()
        # در اینجا می‌توانید دکمه‌های مدیریتی مثل پاک کردن دیتابیس یا تنظیمات را اضافه کنید
        st.write("دسترسی‌های مدیریت فعال است.")
    ...
    if uploaded_file and image_prompt:
        ...
        with st.chat_message("assistant"):
            with st.spinner("در حال تحلیل..."):
                model_list = ["mistralai/pixtral-12b:free", "google/gemini-2.0-flash-lite-preview-02-05:free", "meta-llama/llama-3.2-11b-vision-instruct"]
                success = False
                ...
                if not success: st.error("❌ فعلاً سرورها شلوغ هستند.")
...
if prompt := st.chat_input("پیام یا دستور خود را بنویسید..."):
    ...
    with st.chat_message("assistant"):
        if mode == "🎨 تولید تصویر":
            ...
            url = f"https://image.pollinations.ai/prompt/{urllib.parse.quote(f'Real photo of {prompt}, 8k')}?width=1024&height=1024&seed={seed}"
            ...
            try:
                ...
                st.markdown(res)
                st.session_state.messages.append({"role": "assistant", "content": res})
            except: st.error("خطا در پاسخگویی سرور.")
