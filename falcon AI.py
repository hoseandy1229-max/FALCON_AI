...
def copy_js(text):
    ...
    return f"""<button onclick="navigator.clipboard.writeText(`{safe_text}`)">📋</button>"""
...
# نمایش پیام‌ها با آیکون و تعامل
for i, msg in enumerate(current_messages):
    with st.chat_message(msg["role"], avatar=PERSONA_EMOJIS.get(st.session_state.persona) if msg["role"] == "assistant" else None):
        ...
        if msg["role"] == "assistant" and msg.get("type") != "image_gen":
            ...
            with c1: components.html(copy_js(msg["content"]), height=30)
            with c2: 
                if st.button("👍", key=f"l_{i}"): st.session_state.user_pref += f" [لایک شده: {msg['content'][:20]}]"; st.rerun()
            with c3: 
                if st.button("👎", key=f"d_{i}"): st.session_state.user_pref += f" [دیس‌لایک شده: {msg['content'][:20]}]"; st.rerun()
...
if prompt := st.chat_input("𝑨𝑺𝑲 𝑭𝒂𝒍𝒄𝒐𝒏 𝑨𝑰"):
    ...
    with st.chat_message("assistant", avatar=PERSONA_EMOJIS.get(st.session_state.persona)):
        sys_prompt = f"شخصیت شما: {PERSONAS[st.session_state.persona]}. ترجیحات کاربر بر اساس بازخوردها: {st.session_state.user_pref}. پاسخ‌ها را فارسی بده."
